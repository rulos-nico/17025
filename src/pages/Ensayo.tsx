...
export default function Ensayo() {
  ...
  // Vista activa
  const [viewMode, setViewMode] = useState<'kanban' | 'hierarchy'>('kanban');
  ...
  const ensayosFiltrados = useMemo(() => {
    return ensayos.filter((e: EnsayoType & { clienteId?: string | number }) => {
  ...
  // Handlers de acciones - obtienen el ensayoId del selectedEnsayo
  const handleCambiarEstadoSubmit = async (nuevoEstado: string, comentario?: string) => {
    const ensayoId = modals.selectedEnsayo?.id;
    if (!ensayoId) return;
    try {
  ...
  const handleNovedadSubmit = async (razon: string) => {
    const ensayoId = modals.selectedEnsayo?.id;
    if (!ensayoId) return;
    try {
  ...
  const handleReasignarSubmit = async (tecnicoId: string | number) => {
    const ensayoId = modals.selectedEnsayo?.id;
    if (!ensayoId) return;
    try {
